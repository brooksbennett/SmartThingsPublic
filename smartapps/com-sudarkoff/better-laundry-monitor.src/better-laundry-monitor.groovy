...
definition(
  ...
  description: "Using a switch with powerMonitor capability, monitor the laundry cycle and alert when it's done. 3",
...
preferences {
  ...
  section ("Advanced options", hidden: true, hideable: true) {
    input "cycle_start_power_threshold", "number", title: "start cycle when power raises above (W)", defaultValue: 10, description: "10", required: true
    input "cycle_end_power_threshold", "number", title: "stop cycle when power drops below (W)", defaultValue: 8, description: "8", required: true
  }
